#!/bin/bash
# Run atomic_seen evaluations as two separate conditions:
...
BASE_LOG_DIR="${BASE_LOG_DIR:-$PROJECT_ROOT/logs/robot_interference/atomic_seen/$(date +%Y%m%d_%H%M%S)}"

run_condition() {
  local condition_name="$1"
  shift
  echo "============================================================"
  echo "atomic_seen condition: $condition_name"
...
echo "atomic_seen conditions complete. Logs: $BASE_LOG_DIR"
