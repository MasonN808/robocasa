...
if ! python -c "import torch, zmq" >/dev/null 2>&1; then
    echo "ERROR: torch and pyzmq must be installed in the RoboCasa client environment."
    echo "Install pyzmq in the active client env if needed:"
...
if [[ -z "${LOG_DIR:-}" && -n "${MODEL_PATH:-}" ]]; then
    LOG_DIR="./model_evals/backends/gr00t_n1_5/logs/eval/$(basename "$MODEL_PATH")"
fi
LOG_DIR="${LOG_DIR:-./model_evals/backends/gr00t_n1_5/logs/eval/single/$(date +%m%d_%H%M)}"
mkdir -p "$LOG_DIR"
...
echo "============================================================"
echo "  GR00T N1.5 RoboCasa Client"
echo "  GPUs:         $CUDA_VISIBLE_DEVICES"
echo "  Server:       $GROOT_HOST:$PORT"
...
python -u model_evals/backends/gr00t_n1_5/inference_client.py \
    --host "$GROOT_HOST" \
    --port "$PORT" \
    --task_set $TASK_SET \
    --split "$SPLIT" \
