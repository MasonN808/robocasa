...
# ---------------------------------------------------------------------------
# Tiny helpers (previously lived in world_action_model.trainers.wa_trainer)
# ---------------------------------------------------------------------------

def get_model_path(model_name_or_path):
    """Return a local snapshot path; download from HF Hub if necessary."""
...
def process_transformer(transformer, transformer_cfg):
    """Validate transformer config and optionally enable gradient checkpointing.

    Inference never enables gradient checkpointing, but we keep the signature
    for parity with training-time loading.
    """
    in_channels = transformer_cfg.get('in_channels', transformer.config.in_channels)
    if transformer.config.in_channels != in_channels:
        raise ValueError(
            f"Transformer in_channels mismatch: config {transformer.config.in_channels} != requested {in_channels}"
        )
...
def build_model(
    ...
    transformer = CasualWorldActionTransformer.from_pretrained(
        os.path.join(pretrained, "transformer"), torch_dtype=dtype
    )
    encoder = nn.Sequential(nn.Linear(action_dim, 128), nn.GELU(), nn.Linear(128, 256), nn.GELU(), nn.Linear(256, 3072))
    decoder = nn.Sequential(nn.Linear(3072, 256), nn.GELU(), nn.Linear(256, 128), nn.GELU(), nn.Linear(128, action_dim))
    state_encoder = nn.Sequential(nn.Linear(state_dim, 128), nn.GELU(), nn.Linear(128, 256), nn.GELU(), nn.Linear(256, 3072))
    ...
    transformer = process_transformer(transformer, {})
...
class InferenceServer:
    ...
    async def _handler(self, websocket):
        ...
        # Send metadata
        await websocket.send(packer.pack({"model": "giga-world-policy"}))
...
def parse_args():
    parser = argparse.ArgumentParser(description="GigaWorld-Policy RoboCasa Inference Server")
    parser.add_argument("--model_id", type=str,
                        default="Wan-AI/Wan2.2-TI2V-5B-Diffusers",
                        help="Pretrained Wan model: HuggingFace repo id or local path")
    parser.add_argument("--checkpoint_path", type=str, required=True,
                        help="Path to trained model.pt checkpoint")
    parser.add_argument("--stats_path", type=str, required=True,
                        help="Path to normalization stats JSON (norm_stats_delta.json)")
    ...
    parser.add_argument("--dst_size", type=int, nargs=2, default=[256, 320], help="Image dst_size (W H)")
    # Robocasa-specific post-processing
    ...
    parser.add_argument("--tshape", action="store_true", default=False,
                        help="Use T-shape layout: head view full size + wrist views half size")
    parser.add_argument("--tshape_head_index", type=int, default=0,
                        help="Index of head view in image list (0=agentview_left)")
