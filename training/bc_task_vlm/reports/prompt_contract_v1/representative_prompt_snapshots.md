...
close_hinged_part
  Purpose: Close a hinged door, lid, or articulated head on a fixture.
  Required:
...
close_sliding_part
  Purpose: Push in a sliding drawer or rack on a fixture.
  Required:
    target_id (STRING): Exact symbolic ID of the fixture, receptacle, or destination targeted by the action.
    part_id (STRING): Exact key of the part under initial_state.fixtures[target_id].parts; copy it verbatim (for example door, hinged, lid, or sliding).
...
open_sliding_part
  Purpose: Pull out a sliding drawer or rack on a fixture.
  Required:
    target_id (STRING): Exact symbolic ID of the fixture, receptacle, or destination targeted by the action.
    part_id (STRING): Exact key of the part under initial_state.fixtures[target_id].parts; copy it verbatim (for example door, hinged, lid, or sliding).
...
pick_up_object
  ...
  Optional (otherwise omit):
    source_site_id (STRING): Optional exact sub-site within the source; omit when no named sub-site matters.

place_in_receptacle
  Purpose: Place an object into an interior or container-like receptacle. Use target_site_id separately when the exact basin, bowl, rack, shelf, slot, or tray matters.
  Required:
    object_id (STRING): Exact symbolic ID of the movable object being manipulated.
  At least one required alternative:
    target_id (STRING): Exact symbolic ID of the fixture, receptacle, or destination targeted by the action.
    receptacle_id (STRING): Exact symbolic ID of the destination receptacle.
  Optional (otherwise omit):
    target_id (STRING): Exact symbolic ID of the fixture, receptacle, or destination targeted by the action.
    receptacle_id (STRING): Exact symbolic ID of the destination receptacle.
    target_site_id (STRING): Optional exact sub-site within the destination; omit when no named sub-site matters.
    relative_position (STRING): Optional named relative placement; omit unless the goal requires one.

place_next_to
  Purpose: Place an object adjacent to a reference object or nearby fixture on the same support surface. Use reference_object_id only for movable objects and reference_fixture_id only for fixtures.
  Required:
    object_id (STRING): Exact symbolic ID of the movable object being manipulated.
  At least one required alternative:
    reference_id (STRING): Exact symbolic ID of the object or fixture used as the placement reference.
    reference_object_id (STRING): Exact symbolic ID of the movable object used as the placement reference.
...
place_on_object
  Purpose: Place an object on top of another movable support object such as a plate or cutting_board.
  Required:
    object_id (STRING): Exact symbolic ID of the movable object being manipulated.
  At least one required alternative:
    target_id (STRING): Exact symbolic ID of the fixture, receptacle, or destination targeted by the action.
    support_object_id (STRING): Exact symbolic ID of the movable object supporting the placed object.
  Optional (otherwise omit):
    target_id (STRING): Exact symbolic ID of the fixture, receptacle, or destination targeted by the action.
    relative_position (STRING): Optional named relative placement; omit unless the goal requires one.

place_on_surface
  Purpose: Place an object onto an exposed support surface or attachment seat. Keep the fixture id in support_id or target_id and put the exact burner, rack, or other sub-location in target_site_id.
  Required:
    object_id (STRING): Exact symbolic ID of the movable object being manipulated.
  At least one required alternative:
    target_id (STRING): Exact symbolic ID of the fixture, receptacle, or destination targeted by the action.
    support_id (STRING): Exact symbolic ID of the fixture or surface supporting the placement.
  Optional (otherwise omit):
    target_id (STRING): Exact symbolic ID of the fixture, receptacle, or destination targeted by the action.
    support_id (STRING): Exact symbolic ID of the fixture or surface supporting the placement.
    target_site_id (STRING): Optional exact sub-site within the destination; omit when no named sub-site matters.
...
press_lever
  Purpose: Activate a discrete lever-like control on a fixture.
  Required:
    target_id (STRING): Exact symbolic ID of the fixture, receptacle, or destination targeted by the action.
    control_id (STRING): Exact key of the control under initial_state.fixtures[target_id].controls; copy it verbatim.

set_rotary_control
  Purpose: Set a rotary or handle-like control to an explicit goal state.
  Required:
    target_id (STRING): Exact symbolic ID of the fixture, receptacle, or destination targeted by the action.
    control_id (STRING): Exact key of the control under initial_state.fixtures[target_id].controls; copy it verbatim.
    goal (STRING): Requested final symbolic state of the control.
...
[
  ...
  {
    "function": {
      "description": "Place an object adjacent to a reference object or nearby fixture on the same support surface. Use reference_object_id only for movable objects and reference_fixture_id only for fixtures. Requires at least one of: reference_id or reference_object_id or reference_fixture_id.",
      "name": "place_next_to",
      "parameters": {
        "additionalProperties": false,
        "properties": {
          ...
          "reference_id": {
            "description": "Exact symbolic ID of the object or fixture used as the placement reference.",
            "type": "string"
          },
          "reference_object_id": {
  ...
  {
    "function": {
      "description": "Place an object on top of another movable support object such as a plate or cutting_board. Requires at least one of: target_id or support_object_id.",
...
Carrier: `deliver_straw/traj_000002`
...
```text
Task family: DeliverStraw
Task instruction: Take a straw from the drawer and place it inside the glass cup on the dining counter.
Current acting agent: agent_0
Coordinator for this episode: agent_0
Observation views attached in order: none
...
Carrier: `setup_soda_bowl/traj_000002`
...
```text
Task family: SetupSodaBowl
Task instruction: Grab two sodas from the fridge and place both inside the bowl with ice on the dining counter; open the fridge first if it is closed.
Current acting agent: agent_0
Coordinator for this episode: agent_0
Observation views attached in order: none
...
Carrier: `colorful_salsa/traj_000000`
...
```text
Task family: ColorfulSalsa
Task instruction: Place the avocado, onion, tomato, and bell pepper on the cutting board.
Current acting agent: agent_0
Coordinator for this episode: agent_0
Observation views attached in order: none
...
Carrier: `arrange_tea/traj_000001`
...
```text
Task family: ArrangeTea
Task instruction: Pick the kettle from the counter and place it on the tray. Then pick the mug from the cabinet and place it on the tray. Finally, close the cabinet doors.
Current acting agent: agent_1
Coordinator for this episode: agent_1
Observation views attached in order: none
...
Carrier: `prepare_drink_station/traj_000004`
...
```text
Task family: PrepareDrinkStation
Task instruction: Prepare a drink station by placing the cup and mug on the tray and the pitcher next to the tray on the dining counter.
Current acting agent: agent_0
Coordinator for this episode: agent_0
Observation views attached in order: none
...
Carrier: `spicy_marinade/traj_000000`
...
```text
Task family: SpicyMarinade
Task instruction: Place the bowl and condiment from the cabinet on the counter, opening the cabinet first if it is closed. Then place the lime and garlic on the cutting board.
Current acting agent: agent_0
Coordinator for this episode: agent_0
Observation views attached in order: none
