...
tasks = set(os.environ['TASK_LIST'].splitlines())
...
MAX_PARALLEL=3
NUM_PROC=8
LOG_DIR="$REPO_ROOT/slurm_logs/push_logs_remaining_$$"
mkdir -p "$LOG_DIR"

echo ""
echo "Pushing remaining tasks to HuggingFace (MAX_PARALLEL=$MAX_PARALLEL, NUM_PROC=$NUM_PROC)..."
