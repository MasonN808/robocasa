...
usage() {
  cat <<'EOF'
Usage: sbatch scripts/sbatch/sbatch_image_processing_gpuA100x4_1node_task_cleanup.sh <run_timestamp> [sweep_cli_args...]
...
Example:
  sbatch scripts/sbatch/sbatch_image_processing_gpuA100x4_1node_task_cleanup.sh 20260401T000000Z
...
if [[ -z "${HF_TOKEN:-}" ]]; then
  echo "HF_TOKEN is not set. Export your HuggingFace write token before submitting." >&2
  exit 1
fi
export HF_TOKEN
